...
from operations_agent.graph import run_multi_agent_investigation, run_tool_calling_graph
...
def run_investigation(objective: str | object) -> AgentRunResponse:
    """Run one configured customer investigation and convert it to an API contract."""
    objective_text = getattr(objective, "objective", str(objective))
...
def run_multi_agent_investigation_service(objective: str | object) -> AgentRunResponse:
    """Run a full multi-agent investigation and convert it to an API contract."""
    objective_text = getattr(objective, "objective", str(objective))
    execution_id = str(uuid4())
    ...
    return AgentRunResponse(
        ...
        execution_summary=ExecutionSummary(
            ...
            tool_actions=len(result.completed_actions),
            observations=len(result.observations),
            plan_steps=len(result.agent_results),
            errors=tuple(
                ExecutionErrorSummary(code=error.code, recoverable=error.recoverable)
                for error in result.errors
            ),
